	...
	if (dialogRender())
	{
		...
		case SCHEDULE_WORK:
			if (!_select)
			{
				...
				{
					_teachImg[i].img->frameRender(DC, _teachImg[i].data.rc.left, _teachImg[i].data.rc.top, _teachImg[i].frameX, 0);
					IMAGEMANAGER->findImage("workImg")->frameRender(DC, _teachImg[i].data.rc.left + 5, _teachImg[i].data.rc.top + 7, _teachImg[i].frameY, 0);
					TextOut(DC, _teachImg[i].data.rc.left + 50, _teachImg[i].data.rc.top + 7, _sm->getVTeach()[i]->getName().c_str(), strlen(_sm->getVTeach()[i]->getName().c_str()));
					string str = _sm->getVTeach()[i]->getStringStep() + " " + to_string(_sm->getVTeach()[i]->getTuition()) + "G";
					TextOut(DC, _teachImg[i].data.rc.left + 50, _teachImg[i].data.rc.top + 30, str.c_str(), strlen(str.c_str()));
				}
...
void scheduleScene::setScheduleImage()
{
	for (int i = 0; i < 5; i++)
	{
		for (int j = 0; j < 2; j++)
		{
			_teachImg[j * 5 + i].img = new image;
			_teachImg[j * 5 + i].img->init("image/main/storeSelect(320x52,2x1).bmp", 320, 52, 2, 1, true, RGB(255, 0, 255));
			_teachImg[j * 5 + i].data.rc = RectMake(460 + 170*j, 135 + 60*i, 160, 52);

			_teachImg[j * 5 + i].frameY = j * 5 + i;
	...
	for (int i = 0; i < 8; i++)
	{
		...
		{
			if (i == 7 && j == 1) break;
			_workImg[j * 5 + i].img = new image;
			_workImg[j * 5 + i].img->init("image/main/storeSelect(320x52,2x1).bmp", 320, 52, 2, 1, true, RGB(255, 0, 255));
			_workImg[j * 5 + i].data.rc = RectMake(460 + 170 * j, 435 - 60 * i, 160, 52);
			_workImg[j * 5 + i].frameY = j * 5 + i;

			if (_sm->getVWork()[j * 5 + i]->getAge() >= _princess->getAge())
			{
